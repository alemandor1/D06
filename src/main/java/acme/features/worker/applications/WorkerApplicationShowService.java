...
public class WorkerApplicationShowService implements AbstractShowService<Worker, Application> {
	...
	public boolean authorise(final Request<Application> request) {
		...
		return this.repository.isAuthorised(request.getModel().getInteger("id"), request.getPrincipal().getActiveRoleId());
	}
	...
	public void unbind(final Request<Application> request, final Application entity, final Model model) {
		...
		request.unbind(entity, model, "reference", "moment", "status", "statement", "skills", "qualifications", "job.title");
	}
