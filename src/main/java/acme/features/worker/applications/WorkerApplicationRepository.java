...
public interface WorkerApplicationRepository extends AbstractRepository {
	...
	boolean isAuthorised(int idA, int idU);
	...
	@Query("select count(a) > 0 from Application a where a.id = ?1 and a.worker.id = ?2 and (a.status = ?3 or a.status = ?4)")
	boolean isAuthorisedUpdate(int idA, int idU, ApplicationStatus as1, ApplicationStatus as2);
	...
	Application findApplicationByReference(String reference);
